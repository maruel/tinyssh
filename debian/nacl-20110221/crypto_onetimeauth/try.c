...
const char* checksum_compute(void)
{
    ...
    for (i = 0; i < CHECKSUM_BYTES; ++i) {
        ...
        if (crypto_onetimeauth(h, m, mlen, k) != 0)
            return "crypto_onetimeauth returns nonzero";
        ...
        if (crypto_onetimeauth(m2, m2, mlen, k) != 0)
            return "crypto_onetimeauth returns nonzero";
        for (j = 0; j < hlen; ++j)
            if (m2[j] != h[j])
        ...
        for (j = 0; j < hlen; ++j)
            m2[j] = m[j];
        if (crypto_onetimeauth(k2, m2, mlen, k2) != 0)
            return "crypto_onetimeauth returns nonzero";
        for (j = 0; j < hlen; ++j)
            if (k2[j] != h[j])
        ...
        if (crypto_onetimeauth_verify(h, m, mlen, k) != 0)
            return "crypto_onetimeauth_verify returns nonzero";
    ...
    if (crypto_onetimeauth(h, m, CHECKSUM_BYTES, k) != 0)
        return "crypto_onetimeauth returns nonzero";
    if (crypto_onetimeauth_verify(h, m, CHECKSUM_BYTES, k) != 0)
        return "crypto_onetimeauth_verify returns nonzero";
