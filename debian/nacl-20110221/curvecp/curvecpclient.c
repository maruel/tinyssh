...
long long hellowait[NUMIP] = {
    1000000000LL, 1500000000LL, 2250000000LL, 3375000000LL, 5062500000LL, 7593750000LL, 11390625000LL, 17085937500LL
};
...
{
    if (s)
        die_4(100, USAGE, "curvecpclient: fatal: ", s, "\n");
    die_1(100, USAGE);
}
...
void die_fatal(const char* trouble, const char* d, const char* fn)
{
    ...
    if (d) {
        if (fn)
            die_9(111, "curvecpclient: fatal: ", trouble, " ", d, "/", fn, ": ", e_str(errno), "\n");
        die_7(111, "curvecpclient: fatal: ", trouble, " ", d, ": ", e_str(errno), "\n");
    }
    if (errno)
        die_5(111, "curvecpclient: fatal: ", trouble, ": ", e_str(errno), "\n");
...
int multiipparse(unsigned char* y, const char* x)
{
    ...
    while (ynum < 1000) {
        ...
        for (k = 0; k < 4; ++k) {
            pos = ypos * 4 + k;
            pos2 = (ynum - 1) * 4 + k;
            if (pos >= 0 && pos < 4 * NUMIP && pos2 >= 0 && pos2 < 4 * NUMIP)
                y[pos2] = y[pos];
            d = 0;
            for (j = 0; j < 3 && x[j] >= '0' && x[j] <= '9'; ++j)
                d = d * 10 + (x[j] - '0');
            if (j == 0)
                return 0;
            x += j;
            if (pos >= 0 && pos < 4 * NUMIP)
                y[pos] = d;
            if (k < 3) {
                if (*x != '.')
                    return 0;
...
int main(int argc, char** argv)
{
    ...
    if (!nameparse(servername, *++argv))
        die_usage("sname must be at most 255 bytes, at most 63 bytes between dots");
    if (!hexparse(serverlongtermpk, 32, *++argv))
        die_usage("pk must be exactly 64 hex characters");
    if (!multiipparse(serverip, *++argv))
        die_usage("ip must be a comma-separated series of IPv4 addresses");
    if (!portparse(serverport, *++argv))
        die_usage("port must be an integer between 0 and 65535");
    if (!hexparse(serverextension, 16, *++argv))
        die_usage("ext must be exactly 32 hex characters");
    if (!*++argv)
        die_usage("missing prog");

    for (;;) {
        r = open_read("/dev/null");
        if (r == -1)
            die_fatal("unable to open /dev/null", 0, 0);
        if (r > 9) {
            close(r);
            break;
        }
    ...
    if (keydir) {
        fdwd = open_cwd();
        if (fdwd == -1)
            die_fatal("unable to open current working directory", 0, 0);
        if (chdir(keydir) == -1)
            die_fatal("unable to change to directory", keydir, 0);
        if (load("publickey", clientlongtermpk, sizeof clientlongtermpk) == -1)
            die_fatal("unable to read public key from", keydir, 0);
        if (load(".expertsonly/secretkey", clientlongtermsk, sizeof clientlongtermsk) == -1)
            die_fatal("unable to read secret key from", keydir, 0);
    } else {
    ...
    udpfd = socket_udp();
    if (udpfd == -1)
        die_fatal("unable to create socket", 0, 0);
    ...
    for (hellopackets = 0; hellopackets < NUMIP; ++hellopackets) {
        ...
        for (;;) {
            ...
            p[0].fd = udpfd;
            p[0].events = POLLIN;
            if (poll(p, 1, timeout / 1000000 + 1) < 0)
                p[0].revents = 0;
            ...
            do { /* try receiving a Cookie packet: */
                ...
                if (r != 200)
                    break;
                if (!(byte_isequal(packetip, 4, serverip + 4 * hellopackets) & byte_isequal(packetport, 2, serverport) & byte_isequal(packet, 8, "RL3aNMXK") & byte_isequal(packet + 8, 16, clientextension) & byte_isequal(packet + 24, 16, serverextension)))
                    break;
                byte_copy(nonce, 8, "CurveCPK");
                byte_copy(nonce + 8, 16, packet + 40);
                byte_zero(text, 16);
                byte_copy(text + 16, 144, packet + 56);
                if (crypto_box_open_afternm(text, text, 160, nonce, clientshortserverlong))
                    break;
    ...
    errno = ETIMEDOUT;
    die_fatal("no response from server", 0, 0);
...
receivedcookie:
    ...
    if (keydir) {
        if (safenonce(nonce + 8, 0) == -1)
            die_fatal("nonce-generation disaster", 0, 0);
    } else {
    ...
    if (open_pipe(tochild) == -1)
        die_fatal("unable to create pipe", 0, 0);
    if (open_pipe(fromchild) == -1)
        die_fatal("unable to create pipe", 0, 0);

    child = fork();
    if (child == -1)
        die_fatal("unable to fork", 0, 0);
    if (child == 0) {
        if (keydir)
            if (fchdir(fdwd) == -1)
                die_fatal("unable to chdir to original directory", 0, 0);
        close(8);
        if (dup(tochild[0]) != 8)
            die_fatal("unable to dup", 0, 0);
        close(9);
        if (dup(fromchild[1]) != 9)
            die_fatal("unable to dup", 0, 0);
    ...
    for (;;) {
        ...
        do { /* try receiving a Message packet: */
            ...
            if (flagreceivedmessage && packetnonce <= receivednonce)
                break;
            if (!(byte_isequal(packetip, 4, serverip + 4 * hellopackets) & byte_isequal(packetport, 2, serverport) & byte_isequal(packet, 8, "RL3aNMXM") & byte_isequal(packet + 8, 16, clientextension) & byte_isequal(packet + 24, 16, serverextension)))
                break;
            byte_copy(nonce, 16, "CurveCP-server-M");
            byte_copy(nonce + 16, 8, packet + 40);
            byte_zero(text, 16);
            byte_copy(text + 16, r - 48, packet + 48);
            if (crypto_box_open_afternm(text, text, r - 32, nonce, clientshortservershort))
                break;
            ...
            if (writeall(tochild[1], text + 31, r - 63) == -1)
                goto done;
        } while (0);
        ...
        do { /* try receiving data from child: */
            ...
            for (i = 0; i < childbuflen; ++i) {
                if (childmessagelen < 0)
                    goto done;
                if (childmessagelen >= sizeof childmessage)
                    goto done;
                childmessage[childmessagelen++] = childbuf[i];
                if (childmessage[0] & 128)
                    goto done;
    ...
    if (!WIFEXITED(childstatus)) {
        errno = 0;
        die_fatal("process killed by signal", 0, 0);
    }
