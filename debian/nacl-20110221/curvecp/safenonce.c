...
*/
    ...
    static crypto_uint64 counterlow
    = 0;
...
int safenonce(unsigned char* y, int flaglongterm)
{
    if (!flagkeyloaded) {
        ...
        if (fdlock == -1)
            return -1;
        if (load(".expertsonly/noncekey", noncekey, sizeof noncekey) == -1) {
            close(fdlock);
            return -1;
        }
        close(fdlock);
    ...
    if (counterlow >= counterhigh) {
        ...
        if (fdlock == -1)
            return -1;
        if (load(".expertsonly/noncecounter", data, 8) == -1) {
            close(fdlock);
            return -1;
        }
        ...
        if (savesync(".expertsonly/noncecounter", data, 8) == -1) {
            close(fdlock);
            return -1;
        }
        close(fdlock);
