...
{
    if (s)
        die_4(100, USAGE, "curvecpmessage: fatal: ", s, "\n");
    die_1(100, USAGE);
}
...
void die_fatal(const char* trouble, const char* d, const char* fn)
{
    ...
    if (d) {
        if (fn)
            die_9(111, "curvecpmessage: fatal: ", trouble, " ", d, "/", fn, ": ", e_str(errno), "\n");
        die_7(111, "curvecpmessage: fatal: ", trouble, " ", d, ": ", e_str(errno), "\n");
    }
    if (errno)
        die_5(111, "curvecpmessage: fatal: ", trouble, ": ", e_str(errno), "\n");
...
void earliestblocktime_compute(void) /* XXX: use priority queue */
{
    ...
    for (i = 0; i < blocknum; ++i) {
        ...
        if (blocktime[pos]) {
            ...
            else if (blocktime[pos] < earliestblocktime)
                earliestblocktime = blocktime[pos];
        }
    }
}
...
int main(int argc, char** argv)
{
    ...
    for (;;) {
        ...
        if (tochild[1] < 0)
            watchtochild = 0;
        if (receivewritten >= receivebytes)
            watchtochild = 0;
        if (watchtochild) {
            q->fd = tochild[1];
            q->events = POLLOUT;
        ...
        if (sendeof)
            watchfromchild = 0;
        if (sendbytes + 4096 > sizeof sendbuf)
            watchfromchild = 0;
        if (watchfromchild) {
            q->fd = fromchild[0];
            q->events = POLLIN;
        ...
        if (wantping == 1)
            nextaction = recent + 1000000000;
        if (wantping == 2)
            nextaction = 0;
        if (blocknum < OUTGOING)
            if (!(sendeof ? sendeofprocessed : sendprocessed >= sendbytes))
                if (nextaction > lastblocktime + nsecperblock)
                    nextaction = lastblocktime + nsecperblock;
        if (earliestblocktime) {
            long long nextretry = earliestblocktime + rtt_timeout;
            if (nextretry < lastblocktime + nsecperblock)
                nextretry = lastblocktime + nsecperblock;
            if (nextretry < nextaction)
                nextaction = nextretry;
        }
        ...
        if (pollret < 0) {
            ...
            if (watch8)
                if (!watch8->revents)
                    watch8 = 0;
            if (watchtochild)
                if (!watchtochild->revents)
                    watchtochild = 0;
            if (watchfromchild)
                if (!watchfromchild->revents)
                    watchfromchild = 0;
        }
        ...
        do { /* try receiving data from child: */
            ...
            sendbytes += r;
            if (sendbytes >= 1152921504606846976LL)
                die_internalerror();
        } while (0);
        ...
        do { /* try re-sending an old block: */
            if (recent < lastblocktime + nsecperblock)
                break;
            if (earliestblocktime == 0)
                break;
            if (recent < earliestblocktime + rtt_timeout)
                break;
        ...
        do { /* try sending a new block: */
            ...
            if (blocklen[pos] > maxblocklen)
                blocklen[pos] = maxblocklen;
        ...
        sendblock:
            ...
            if (u <= 192)
                ...
                die_internalerror();
            if (blocklen[pos] < 0 || blocklen[pos] > 1024)
                die_internalerror();
            ...
            if (writeall(9, buf + 7, u + 1) == -1)
                die_fatal("unable to write descriptor 9", 0, 0);
        ...
        do { /* try processing a message: */
            if (!messagenum)
                break;
            if (tochild[1] >= 0 && receivewritten < receivebytes)
                break;
            ...
            do { /* handle this message if it's comprehensible: */
                ...
                for (i = 0; i < blocknum; ++i) {
                    ...
                    if (blockid[k] == id) {
                        ...
                        if (rtt_delta > 0)
                            rtt_lowwater += rtt_delta / 8192;
                        else
                            rtt_lowwater += rtt_delta / 256;

                        if (rtt_average > rtt_highwater + 5000000)
                            rtt_seenrecenthigh = 1;
                        else if (rtt_average < rtt_lowwater)
                            rtt_seenrecentlow = 1;
                        ...
                        do {
                            if (recent - lastedge < 60000000000LL) {
                                if (recent < lastdoubling + 4 * nsecperblock + 64 * rtt_timeout + 5000000000LL)
                                    break;
                            } else {
                                if (recent < lastdoubling + 4 * nsecperblock + 2 * rtt_timeout)
                                    break;
                            }
                            if (nsecperblock <= 65535)
                                break;
                ...
                for (;;) {
                    if (receivebytes >= receivewritten + sizeof receivebuf)
                        break;
                    if (!receivevalid[receivebytes & (sizeof receivebuf - 1)])
                        break;
                    ++receivebytes;
                }

                if (!uint32_unpack(message[pos]))
                    break; /* never acknowledge a pure acknowledgment */
        ...
        do { /* try sending data to child: */
            ...
            if (pos + len > sizeof receivebuf)
                len = sizeof receivebuf - pos;
