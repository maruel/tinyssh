...
{
    if (s)
        die_4(100, USAGE, "curvecpserver: fatal: ", s, "\n");
    die_1(100, USAGE);
}
...
void die_fatal(const char* trouble, const char* d, const char* fn)
{
    ...
    if (d) {
        if (fn)
            die_9(111, "curvecpserver: fatal: ", trouble, " ", d, "/", fn, ": ", e_str(errno), "\n");
...
int main(int argc, char** argv)
{
    ...
    for (;;) {
        ...
        while (*++x) {
            ...
            if (*x == 'c') {
                if (x[1]) {
                    strmaxactiveclients = x + 1;
                    break;
                }
                if (argv[1]) {
                    strmaxactiveclients = *++argv;
                    break;
                }
            }
            die_usage(0);
    ...
    if (!maxparse(&maxactiveclients, strmaxactiveclients))
        die_usage("concurrency must be between 1 and 65535");
    if (!nameparse(servername, *++argv))
        die_usage("sname must be at most 255 bytes, at most 63 bytes between dots");
    keydir = *++argv;
    if (!keydir)
        die_usage("missing keydir");
    if (!ipparse(serverip, *++argv))
        die_usage("ip must be an IPv4 address");
    ...
    if (!activeclients)
        die_fatal("unable to create activeclients array", 0, 0);
    ...
    if (!p)
        die_fatal("unable to create poll array", 0, 0);

    fdwd = open_cwd();
    if (fdwd == -1)
        die_fatal("unable to open current directory", 0, 0);

    if (chdir(keydir) == -1)
        die_fatal("unable to chdir to", keydir, 0);
    if (load(".expertsonly/secretkey", serverlongtermsk, sizeof serverlongtermsk) == -1)
    ...
    if (socket_bind(udpfd, serverip, serverport) == -1)
        die_fatal("unable to bind socket", 0, 0);
    ...
    for (;;) {
        ...
        if (poll(p, 1 + numactiveclients, timeout / 1000000 + 1) < 0)
            continue;

        do { /* try receiving a packet: */
            if (!p[numactiveclients].revents)
                break;
            ...
            if (r & 15)
                break;
            if (!(byte_isequal(packet, 7, "QvnQ5Xl") & byte_isequal(packet + 8, 16, serverextension)))
                break;
            ...
            if (packet[7] == 'H') { /* Hello packet: */
                ...
                if (crypto_box_open_afternm(text, text, 96, nonce, clientshortserverlong))
                    break;
            ...
            if (packet[7] == 'I') { /* Initiate packet: */
                ...
                if (i < numactiveclients) {
                    ...
                    if (packetnonce <= activeclients[i].receivednonce)
                        break;
                    byte_copy(nonce, 16, "CurveCP-client-I");
                    byte_copy(nonce + 16, 8, packet + 168);
                    byte_zero(text, 16);
                    byte_copy(text + 16, r - 176, packet + 176);
                    if (crypto_box_open_afternm(text, text, r - 160, nonce, activeclients[i].clientshortservershort))
                        break;
                ...
                if (crypto_secretbox_open(text, text, 96, nonce, minutekey)) {
                    byte_zero(text, 16);
                    byte_copy(text + 16, 80, packet + 88);
                    if (crypto_secretbox_open(text, text, 96, nonce, lastminutekey))
                        break;
                }
                if (!byte_isequal(packet + 40, 32, text + 32))
                    break;
                ...
                byte_zero(text, 16);
                byte_copy(text + 16, r - 176, packet + 176);
                if (crypto_box_open_afternm(text, text, r - 160, nonce, clientshortservershort))
                    break;

                if (!byte_isequal(text + 128, 256, servername))
                    break;
                ...
                if (crypto_box_open_afternm(text + 64, text + 64, 64, nonce, clientlongserverlong))
                    break;
                if (!byte_isequal(text + 96, 32, clientshorttermpk))
                    break;
                ...
                activeclients[i].tochild = pi0[1];
                close(pi0[0]);
                activeclients[i].fromchild = pi1[0];
                close(pi1[1]);
            ...
            if (packet[7] == 'M') { /* Message packet: */
                ...
                if (i < numactiveclients) {
                    ...
                    if (crypto_box_open_afternm(text, text, r - 64, nonce, activeclients[i].clientshortservershort))
                        break;
        ...
        for (i = numactiveclients - 1; i >= 0; --i) {
            ...
            endconnection:
                ...
                close(activeclients[i].fromchild);
                activeclients[i].fromchild = -1;
                close(activeclients[i].tochild);
                activeclients[i].tochild = -1;
                --numactiveclients;
