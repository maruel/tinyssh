...
const char* checksum_compute(void)
{
    ...
    for (i = 0; i < CHECKSUM_BYTES; ++i) {
        ...
        if (crypto_auth(h, m, mlen, k) != 0)
            return "crypto_auth returns nonzero";
        ...
        for (j = 0; j < hlen; ++j)
            m2[j] = m[j];
        if (crypto_auth(k2, m2, mlen, k2) != 0)
            return "crypto_auth returns nonzero";
        for (j = 0; j < hlen; ++j)
            if (k2[j] != h[j])
        ...
        if (crypto_auth_verify(h, m, mlen, k) != 0)
            return "crypto_auth_verify returns nonzero";
    ...
    if (crypto_auth(h, m, CHECKSUM_BYTES, k) != 0)
        return "crypto_auth returns nonzero";
    if (crypto_auth_verify(h, m, CHECKSUM_BYTES, k) != 0)
        return "crypto_auth_verify returns nonzero";
