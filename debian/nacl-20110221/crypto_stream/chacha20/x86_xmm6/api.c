...
} ECRYPT_chacha20_x86_xmm6_ctx;

extern void ECRYPT_chacha20_x86_xmm6_encrypt_bytes(ECRYPT_chacha20_x86_xmm6_ctx*, const unsigned char*, unsigned char*, crypto_uint32);
    ...
    const unsigned char* k)
{
    ...
    unsigned char cspace[sizeof(ECRYPT_chacha20_x86_xmm6_ctx) + 16];
    ECRYPT_chacha20_x86_xmm6_ctx* ctx = (ECRYPT_chacha20_x86_xmm6_ctx*)cspace;
    ...
    while (l >= 256) {
        for (i = 0; i < 256; ++i)
            x[i] = m[i];
        ECRYPT_chacha20_x86_xmm6_encrypt_bytes(ctx, x, x, 256);
        for (i = 0; i < 256; ++i)
            c[i] = x[i];
    ...
    if (l > 0) {
        for (i = 0; i < l; ++i)
            x[i] = m[i];
        ECRYPT_chacha20_x86_xmm6_encrypt_bytes(ctx, x, x, l);
        for (i = 0; i < l; ++i)
            c[i] = x[i];
    }
    for (i = 0; i < sizeof xspace; ++i)
        xspace[i] = 0;
    for (i = 0; i < sizeof cspace; ++i)
        cspace[i] = 0;
    return 0;
}
