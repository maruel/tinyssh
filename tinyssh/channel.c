...
int channel_open(const char* user, crypto_uint32 id, crypto_uint32 remotewindow, crypto_uint32 maxpacket, crypto_uint32* localwindow)
{
    ...
    /* copy user-name */
    if (!str_copyn(channel.user, sizeof channel.user, user))
        bug_nomem();
    ...
    if (!newenv_copyenv("PATH"))
        if (!newenv_env("PATH", "/bin:/usr/bin"))
            return 0;
    ...
    buf_putnum8(&b, 0);
    if (!newenv_env("SSH_CONNECTION", (char*)b.buf))
        return 0;
...
int channel_openterminal(const char* name, crypto_uint32 a, crypto_uint32 b, crypto_uint32 x, crypto_uint32 y)
{
    ...
    if (channel.pid != 0)
        bug_proto();
    if (channel.flagterminal == 1)
        bug_proto();

    if (!channel_openpty(&channel.master, &channel.slave))
        return 0;
...
int channel_exec(const char* cmd)
{
    ...
    if (channel.pid == 0) {
        ...
        if (!channel_droppriv(channel.user, &shell))
            _exit(111);
        if (cmd) {
            run[0] = shell;
            run[1] = (char*)"-c";
            ...
            if (!loginshell(ln, sizeof ln, shell))
                bug();
            run[0] = ln;
            run[1] = 0;
        }
...
void channel_put(unsigned char* buf, long long len)
{
    ...
    if (!buf || len < 0)
        bug_inval();
    if (channel.len0 + len > CHANNEL_BUFSIZE)
        bug_nomem();
...
long long channel_read(unsigned char* buf, long long len)
{
    ...
    if (r > 1048576)
        r = 1048576;
    if (r > channel.remotewindow)
        r = channel.remotewindow;
...
int channel_readisready(void)
{
    ...
    if (channel.maxpacket == 0 || channel.pid == 0)
        return 0;
    if (channel.fd1 == -1)
        return 0;
...
int channel_waitnohang(int* s, int* e)
{
    ...
    if (channel.flagterminal)
        logsys_logout(channel.user, channel.remoteip, channel.termname, channel.pid);
