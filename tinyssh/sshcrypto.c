...
void sshcrypto_init(void)
{
    ...
    for (i = 0; sshcrypto_kexs[i].name; ++i) {
        if (sshcrypto_kexs[i].dh_publickeybytes > sshcrypto_dh_PUBLICKEYMAX)
            bug_inval();
        if (sshcrypto_kexs[i].dh_secretkeybytes > sshcrypto_dh_SECRETKEYMAX)
            bug_inval();
        if (sshcrypto_kexs[i].dh_bytes > sshcrypto_dh_MAX)
            bug_inval();
        if (sshcrypto_kexs[i].hash_bytes > sshcrypto_hash_MAX)
            bug_inval();
    }
    ...
    for (i = 0; sshcrypto_keys[i].name; ++i) {
        ...
        if (sshcrypto_keys[i].sign_secretkeybytes > sshcrypto_sign_SECRETKEYMAX)
            bug_inval();
        if (sshcrypto_keys[i].sign_bytes > sshcrypto_sign_MAX)
            bug_inval();
