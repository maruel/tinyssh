...
int packet_auth(struct buf* b, struct buf* b2)
{
    ...
    /* parse "ssh-userauth" */
    pos = 0;
    if (!packet_getall(b, SSH_MSG_SERVICE_REQUEST))
        bug();
    ...
    if (!byte_isequal(b->buf + pos - len, len, "ssh-userauth"))
        bug_proto();
    ...
    for (count = 0; count < 32; ++count) {
        ...
        pos = 0;
        buf_purge(b);
        if (!packet_getall(b, SSH_MSG_USERAUTH_REQUEST))
            bug();
        ...
        if (len != 14)
            bug_proto();
        pos = packetparser_skip(b->buf, b->len, pos, len);
        if (!byte_isequal(b->buf + pos - len, len, "ssh-connection"))
            bug_proto();
        ...
        if (str_equaln((char*)b->buf + pos - len, len, "publickey")) {
            ...
            for (i = 0; sshcrypto_keys[i].name; ++i) {
                if (!sshcrypto_keys[i].sign_flagclient)
                    continue;
                if (!str_equaln(pkname, len, sshcrypto_keys[i].name))
                    continue;
            ...
            if (sign_open && parsesignpk && putsignpk && putsignpkbase64 && parsesignature) {
                ...
                pos = packetparser_skip(b->buf, b->len, pos, len);
                if (!parsesignpk(pk, b->buf + pos - len, len))
                    bug_proto();
                ...
                pos = packetparser_skip(b->buf, b->len, pos, len);
                if (!parsesignature(sig, b->buf + pos - len, len))
                    bug_proto();
                pos = packetparser_end(b->buf, b->len, pos);
