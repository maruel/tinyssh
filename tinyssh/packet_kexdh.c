...
int packet_kexdh(const char* keydir, struct buf* b1, struct buf* b2)
{
    ...
    if (packet.kex_packet_follows && !packet.kex_guess) {
        buf_purge(b1);
        if (!packet_getall(b1, SSH_MSG_KEXDH_INIT))
            bug_proto();
    }
    ...
    if (len != sshcrypto_dh_publickeybytes)
        bug_proto();
    ...
    do {
        ...
        if (sshcrypto_dh_keypair(serverpk, serversk) != 0)
            bug_proto();
        if (sshcrypto_dh(sharedsecret, clientpk, serversk) != 0)
            bug_proto();
    ...
    /* signature */
    if (subprocess_sign(sm, sshcrypto_sign_bytes, keydir, hash, sshcrypto_hash_bytes) != 0)
        bug();
    buf_purge(b1);
    buf_purge(b2);
    ...
    for (i = 0; i < 6; ++i) {
        ...
        if (i == 0)
            byte_copy(packet.clientnonce, sshcrypto_cipher_KEYMAX, key);
        if (i == 1)
            byte_copy(packet.servernonce, sshcrypto_cipher_KEYMAX, key);
        if (i == 2)
            byte_copy(packet.clientkey, sshcrypto_cipher_KEYMAX, key);
        if (i == 3)
            byte_copy(packet.serverkey, sshcrypto_cipher_KEYMAX, key);
        if (i == 4)
