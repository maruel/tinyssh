...
int packet_kex_receive(void)
{
    ...
    pos = packetparser_skip(b->buf, b->len, pos, len);
    if (!sshcrypto_key_select(b->buf + pos - len, len))
        bug_proto();

    pos = packetparser_uint32(b->buf, b->len, pos, &len); /* encryption algorithms client to server */
    pos = packetparser_skip(b->buf, b->len, pos, len);
    if (!sshcrypto_cipher_select(b->buf + pos - len, len))
        bug_proto();
    ...
    pos = packetparser_skip(b->buf, b->len, pos, len);
    if (!sshcrypto_cipher_macselect(b->buf + pos - len, len))
        bug_proto();
