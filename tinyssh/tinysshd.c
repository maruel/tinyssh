...
\n\
 options:\n\
   ...
   -o (optional): enable older standardized crypto - ecdsa-sha2-nistp256, ecdh-sha2-nistp256, aes128-ctr, aes256-ctr, hmac-sha2-256\n\
...
int main(int argc, char** argv)
{
    ...
    for (i = 0; sshcrypto_keys[i].name; ++i)
        sshcrypto_keys[i].sign_flagserver |= sshcrypto_kexs[i].cryptotype & cryptotypeselected;
    for (i = 0; sshcrypto_keys[i].name; ++i)
        sshcrypto_keys[i].sign_flagclient |= sshcrypto_kexs[i].cryptotype & cryptotypeselected;
    for (i = 0; sshcrypto_kexs[i].name; ++i)
        sshcrypto_kexs[i].flagenabled |= sshcrypto_kexs[i].cryptotype & cryptotypeselected;
    for (i = 0; sshcrypto_ciphers[i].name; ++i)
        sshcrypto_ciphers[i].flagenabled |= sshcrypto_ciphers[i].cryptotype & cryptotypeselected;
    ...
    if (!packet_hello_send())
        die_fatal("unable to send hello-string", 0, 0);
    if (!packet_hello_receive())
        die_fatal("unable to receive hello-string", 0, 0);

    /* send and receive kex */
    if (!packet_kex_send())
        die_fatal("unable to send kex-message", 0, 0);
    if (!packet_kex_receive())
...
rekeying:
    ...
    if (!packet_kexdh(keydir, &b1, &b2))
        die_fatal("unable to subprocess kexdh", 0, 0);

    if (packet.flagkeys)
        log_d1("rekeying: done");
    packet.flagkeys = 1;
    ...
    for (;;) {
        ...
        if (poll(p, q - p, 60000) < 0) {
            ...
            if (watch1)
                if (!watch1->revents)
                    watch1 = 0;
            if (watchfromchild1)
                if (!watchfromchild1->revents)
                    watchfromchild1 = 0;
            if (watchfromchild2)
                if (!watchfromchild2->revents)
                    watchfromchild2 = 0;
            if (watchtochild)
                if (!watchtochild->revents)
                    watchtochild = 0;
            if (watchselfpipe)
                if (!watchselfpipe->revents)
                    watchselfpipe = 0;
        }
        ...
        if (watchfromchild2)
            packet_channel_send_extendeddata(&b2);
        ...
        for (;;) {
            ...
            case SSH_MSG_CHANNEL_REQUEST:
                if (!packet_channel_request(&b1, &b2))
            ...
            case SSH_MSG_CHANNEL_EXTENDED_DATA:
                if (!packet_channel_recv_extendeddata(&b1))
                    die_fatal("unable to handle channel-extended-data", 0, 0);
                break;
            case SSH_MSG_CHANNEL_WINDOW_ADJUST:
                if (!packet_channel_recv_windowadjust(&b1))
                    die_fatal("unable to handle channel-window-adjust", 0, 0);
                break;
            ...
            default:
                if (!packet_unimplemented(&b1))
                    die_fatal("unable to send SSH_MSG_UNIMPLEMENTED message", 0, 0);
            }
        }
    }
