    ...
    const unsigned char* buf, long long len, long long pos, crypto_uint8* out)
{
    ...
    if (!buf || len < 0 || len > 1073741824 || pos < 0 || pos > 1073741824 || !out)
        bug_inval_(fn, line);
    if (pos + 1 > len)
        bug_proto_(fn, line);
    *out = buf[pos];
    ...
    const unsigned char* buf, long long len, long long pos, unsigned char* out, long long outlen)
{
    ...
    if (!buf || pos < 0 || len < 0)
        bug_inval_(fn, line);
    if (!buf || len < 0 || len > 1073741824 || pos < 0 || pos > 1073741824 || !out || outlen < 0 || outlen > 1073741824)
        bug_inval_(fn, line);
    if (pos + outlen > len)
        bug_proto_(fn, line);
    ...
    const unsigned char* buf, long long len, long long pos, long long skip)
{
    ...
    if (!buf || len < 0 || len > 1073741824 || pos < 0 || pos > 1073741824 || skip < 0 || skip > 1073741824)
        bug_inval_(fn, line);
    if (pos + skip > len)
        bug_proto_(fn, line);
    return pos + skip;
