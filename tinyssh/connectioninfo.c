...
static int connectioninfo_fromtcpserverenv(char* localip, char* localport, char* remoteip, char* remoteport)
{
    ...
    if (!env(localip, IPTOSTR_LEN, "TCPLOCALIP"))
        return 0;
    if (!env(localport, PORTTOSTR_LEN, "TCPLOCALPORT"))
        return 0;
    if (!env(remoteip, IPTOSTR_LEN, "TCPREMOTEIP"))
        return 0;
    if (!env(remoteport, PORTTOSTR_LEN, "TCPREMOTEPORT"))
        return 0;
    return 1;
}
...
static int connectioninfo_fromcurvecpserverenv(char* localip, char* localport, char* remoteip, char* remoteport)
{
    ...
    if (!env(localip, IPTOSTR_LEN, "CURVECPLOCALIP"))
        return 0;
    if (!env(localport, PORTTOSTR_LEN, "CURVECPLOCALPORT"))
        return 0;
    if (!env(remoteip, IPTOSTR_LEN, "CURVECPREMOTEIP"))
        return 0;
    if (!env(remoteport, PORTTOSTR_LEN, "CURVECPREMOTEPORT"))
        return 0;
    return 1;
}
...
{
    if (connectioninfo_fromtcpserverenv(localip, localport, remoteip, remoteport))
        return;
    if (connectioninfo_fromcurvecpserverenv(localip, localport, remoteip, remoteport))
        return;
