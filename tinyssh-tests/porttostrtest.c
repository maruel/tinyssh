...
static void porttostr_bufferoverflow(void)
{
    ...
    if ((PORTTOSTR_LEN) < sizeof("65535"))
        fail("PORTTOSTR_LEN too small");
    ...
    for (j = 0; j < 2; ++j) {
        ...
        for (i = 0; i < 8; ++i)
            if (buf[i] != ch[j])
                fail("porttostr writes before output");
        for (i = 0; i < 8; ++i)
            if (buf[i + PORTTOSTR_LEN + 8] != ch[j])
                fail("porttostr writes after output");
    }
}
