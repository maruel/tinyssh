...
static void test_vector(void)
{
    ...
    if (crypto_scalarmult_nistp256(r, d, S) != 0)
        fail("crypto_scalarmult_nistp256() failure");
    for (j = 0; j < BYTES; ++j)
        if (r[j] != R[j])
            fail("crypto_scalarmult_nistp256() failure");
}
...
static void test_random(void)
{
    ...
    for (i = 0; i < 16; ++i) {
        ...
        if (crypto_scalarmult_nistp256_base(pk1 + i, sk1 + i) != 0)
            goto fail;
        if (crypto_scalarmult_nistp256_base(pk2 + i, sk2 + i) != 0)
            goto fail;
        if (crypto_scalarmult_nistp256(k1 + i, sk1 + i, pk2 + i) != 0)
            goto fail;
        if (crypto_scalarmult_nistp256(k2 + i, sk2 + i, pk1 + i) != 0)
            goto fail;
        for (j = 0; j < BYTES; ++j)
            if (k1[j + i] != k2[j + i])
                goto fail;
