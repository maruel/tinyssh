...
static void run(unsigned char* x, long long xlen)
{
    ...
    if (pid == 0) {
        ...
        if (writeall(fromchild[1], buf, sizeof buf) == -1)
            _exit(111);
        _exit(0);
    }
    close(fromchild[1]);

    if (readall(fromchild[0], x, xlen) == -1)
        fail("read() failure");
...
int main(void)
{
    ...
    if (verify(buf1, buf2) == 0)
        fail("randombytes() cryptographically dangerous");
    ...
    run(buf2, sizeof buf2);
    if (verify(buf1, buf2) == 0)
        fail("randombytes() cryptographically dangerous across fork()");
